...
class ARedisHash extends ARedisIterableEntity {
	...
	public function add($key, $value) {
		...
		if ($this->getClient()->hset($this->name,$key, $value) === false) {
			return false;
		}
		$this->_data = null;
		$this->_count = null;
	 ...
	 * @return boolean true if the item was added, otherwise false
	 */
	public function aset($key, $value, $expire=0) {
		if ($this->name === null) {
			throw new CException(get_class($this)." requires a name!");
		}
		if ($this->getClient()->hset($this->name,$key, $value) === false) {
            if($expire != 0)
                $this->getClient()->expire($this->name, $expire);
			return false;
		}
		$this->_data = null;
		$this->_count = null;
	...
	public function remove($key) {
		...
		if (!$this->getClient()->hdel($this->name,$key)) {
			return false;
		}
		$this->_data = null;
		$this->_count = null;
	...
	public function getCount() {
		if ($this->_count === null) {
			...
			$this->_count = $this->getClient()->hlen($this->name);
		}
		return $this->_count;
	}
	/**
	...
	public function getData($forceRefresh = false) {
		if ($forceRefresh || $this->_data === null) {
			...
			$this->_data = $this->getClient()->hgetall($this->name);
		}
		return $this->_data;
	}
	...
	public function mSet($fields) {
		...
		if ($this->getClient()->hMset($this->name, $fields) === false) {
			return false;
		}
		$this->_data = null;
		$this->_count = null;
	...
	public function increment($key, $value) {
		...
		if ($this->getClient()->hIncrBy($this->name, $key, (int)$value) === false) {
			return false;
		}
		$this->_data = null;
		$this->_count = null;
