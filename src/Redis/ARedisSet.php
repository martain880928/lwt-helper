...
use LwtHelper\Redis\ARedisIterableEntity;
...
class ARedisSet extends ARedisIterableEntity{
	...
	public function add($item) {
		if (!$this->getClient()->sadd($this->name,$item)) {
			return false;
		}
		$this->_data = null;
		$this->_count = null;
	...
	public function remove($item) {
		if (!$this->getClient()->srem($this->name,$item)) {
			return false;
		}
		$this->_data = null;
		$this->_count = null;
	 ...
	 */
	public function pop() {
		$member = $this->getClient()->spop($this->name);
		$this->_data = null;
		$this->_count = null;
		return $member;
	}
	...
	public function random() {
		return $this->getClient()->srandmember($this->name);
	}
	...
	public function diffStore($destination, $set) {
		if ($destination instanceof ARedisSet) {
			...
			$destination = new ARedisSet($destination,$this);
		}
		if (is_array($set)) {
			$parameters = $set;
		}
	...
	public function move($destination, $item) {
		...
		$this->_count = null;
		$this->_data = null;
		return $this->getClient()->smove($this->name, $destination, $item);
	}
	...
	public function getCount() {
		if ($this->_count === null) {
			$this->_count = $this->getClient()->scard($this->name);
		}
		return $this->_count;
	}
	/**
	...
	public function getData($forceRefresh = false) {
		if ($forceRefresh || $this->_data === null) {
			$this->_data = $this->getClient()->smembers($this->name);
		}
		return $this->_data;
	}
	...
	public function contains($item) {
		return $this->getClient()->sismember($this->name, $item);
	}
	/**
