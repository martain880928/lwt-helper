...
class ARedisSortedSet extends ARedisIterableEntity {
	...
	public function add($key, $value) {
		if (!$this->getClient()->zadd($this->name,$value, $key)) {
			return false;
		}
		$this->_data = null;
		$this->_count = null;
	...
	public function remove($key) {
		if (!$this->getClient()->zrem($this->name,$key)) {
			return false;
		}
		$this->_data = null;
		$this->_count = null;
	...
	{
		if (!($score = $this->getClient()->zincrby($this->name, $byAmount, $key))) {
			return false;
		}
		$this->_data = null;
		$this->_count = null;
	...
	public function interStore($destination, $set, $weights = null) {
		if ($destination instanceof ARedisSortedSet) {
			...
			$destination = new ARedisSortedSet($destination,$this);
		}
		if (is_array($set)) {
			$sets = $set;
		}
	...
	public function getCount() {
		if ($this->_count === null) {
			$this->_count = $this->getClient()->zcard($this->name);
		}
		return $this->_count;
	}
	/**
	...
	public function getData($forceRefresh = false) {
		if ($forceRefresh || $this->_data === null) {
			$this->_data = $this->getClient()->zrange($this->name,0, -1, true);
		}
		return $this->_data;
	}
	...
	public function getScore($key) {
		$score = $this->getClient()->zScore($this->name,$key);
		if (!$score) {
			return false;
		}
		return $score;
