...
class ARedisList extends ARedisIterableEntity {
	...
	public function add($item) {
		...
		if (!$this->getClient()->rpush($this->name,$item)) {
			return false;
		}
		$this->_data = null;
		$this->_count = null;
	...
	public function remove($item) {
		...
		if (!$this->getClient()->lrem($this->name,$item,1)) {
			return false;
		}
		$this->_data = null;
		$this->_count = null;
	...
	public function unshift($item) {
		...
		if (!$this->getClient()->lpush($this->name,$item)) {
			return false;
		}
		$this->_data = null;
		$this->_count = null;
	...
	public function shift() {
		...
		$item = $this->getClient()->lpop($this->name);
		$this->_data = null;
		$this->_count = null;
		return $item;
	}
	...
	public function pop() {
		...
		$item = $this->getClient()->rpop($this->name);
		$this->_data = null;
		$this->_count = null;
		return $item;
	}
	...
	public function range($start = 0, $stop = -1) {
		...
		return $this->getClient()->lrange($this->name, $start, $stop);
	}
	/**
	...
	public function trim($start, $stop) {
		...
		return $this->getClient()->ltrim($this->name, $start, $stop) ? true : false;
	}
	...
	public function getCount() {
		if ($this->_count === null) {
			...
			$this->_count = (int) $this->getClient()->lSize($this->name);
		}
		return $this->_count;
	}
	/**
